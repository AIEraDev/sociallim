...
const emailProviderFactory_1 = require("./email/emailProviderFactory");
class EmailService {
    static async sendVerificationEmail(email, verificationToken) {
        const verificationUrl = `${environment_1.config.frontendUrl}/auth/verify-email?token=${verificationToken}`;
        try {
          ...
          `);
            ...
            console.log("Verification email sent successfully:", {
                provider: environment_1.config.email.provider,
    ...
    static async sendPasswordResetEmail(email, resetToken) {
        const resetUrl = `${environment_1.config.frontendUrl}/auth/reset-password?token=${resetToken}`;
        try {
            const result = await this.provider.sendEmail({
                from: this.fromEmail,
                to: [email],
    ...
    static async renderPasswordResetEmailTemplate(email, resetUrl) {
        return `
    <!DOCTYPE html>
    <html>
    <head>
    ...
    static async renderWelcomeEmailTemplate(email, name) {
        return `
    <!DOCTYPE html>
    <html>
    <head>
    ...
    <body>
      <div class="container">
        ...
        <div class="content">
          ...
          <p style="text-align: center;">
            <a href="${environment_1.config.frontendUrl}/dashboard" class="button">Get Started</a>
          </p>
    ...
    static async renderVerificationEmailTemplate(email, verificationUrl) {
        return `
    <!DOCTYPE html>
    <html>
    <head>
...
EmailService.provider = emailProviderFactory_1.EmailProviderFactory.createProvider();
EmailService.fromEmail = environment_1.config.email.fromEmail;
