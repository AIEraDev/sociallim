...
import { Comment, Platform } from "@prisma/client";

describe("CommentPreprocessor", () => {
  let preprocessor: CommentPreprocessor;

  beforeEach(() => {
    preprocessor = new CommentPreprocessor();
  });
  ...
  describe("preprocessComments", () => {
    ...
    it("should detect duplicate comments", async () => {
      const comments = [
        ...
        createMockComment("This is a great video", "2"), // Exact duplicate
        createMockComment("This is great video", "3"), // Similar
  ...
  describe("text cleaning and normalization", () => {
    it("should clean and normalize text properly", async () => {
      ...
      // All should be processed (assuming they don't trigger spam/toxic filters)
  ...
  describe("edge cases", () => {
    ...
    it("should handle comments with only special characters", async () => {
      ...
      const result = await preprocessor.preprocessComments(comments);
      // These should likely be filtered as spam
      expect(result.spamComments.length).toBeGreaterThan(0);
    });
    ...
    it("should handle mixed language content", async () => {
      ...
      const result = await preprocessor.preprocessComments(comments);
      // These should pass through as valid comments
