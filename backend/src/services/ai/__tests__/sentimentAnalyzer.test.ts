...
import { Comment, Platform, Sentiment } from "@prisma/client";
...
describe("SentimentAnalyzer", () => {
  ...
  const createMockComment = (text: string, id: string = "1"): Comment => ({
    id,
  ...
  describe("constructor", () => {
    it("should throw error if no API key is provided", () => {
      ...
      expect(() => new SentimentAnalyzer()).toThrow("Gemini API key is required");
    });

    it("should accept API key as parameter", () => {
      delete process.env.GEMINI_API_KEY;

      expect(() => new SentimentAnalyzer("custom-key")).not.toThrow();
    });
  });
...
{"commentIndex": 3, "sentiment": "NEUTRAL", "confidence": 0.6, "emotions": [{"name": "trust", "score": 0.4}]}
      ...
      expect(result.results[0].sentiment).toBe(Sentiment.POSITIVE);
      expect(result.results[1].sentiment).toBe(Sentiment.NEGATIVE);
      expect(result.results[2].sentiment).toBe(Sentiment.NEUTRAL);
    ...
    it("should handle API errors with fallback results", async () => {
      ...
      expect(result.results[0].confidence).toBeLessThan(0.7); // Fallback has lower confidence
  ...
  describe("analyzeSingleComment", () => {
    it("should analyze a single comment", async () => {
      ...
      expect(result.confidence).toBeGreaterThan(0.3); // Allow for fallback results
  ...
  describe("validateResults", () => {
    ...
    it("should detect low confidence issues", () => {
      const results = [
        { sentiment: Sentiment.POSITIVE, confidence: 0.1, emotions: [] },
        { sentiment: Sentiment.NEGATIVE, confidence: 0.2, emotions: [] },
        { sentiment: Sentiment.NEUTRAL, confidence: 0.15, emotions: [] },
      ];
    ...
    it("should detect missing emotion data", () => {
      const results = [
        { sentiment: Sentiment.POSITIVE, confidence: 0.8, emotions: [] },
        { sentiment: Sentiment.NEGATIVE, confidence: 0.7, emotions: [] },
        { sentiment: Sentiment.NEUTRAL, confidence: 0.6, emotions: [] },
      ];
    ...
    it("should pass validation for good results", () => {
      const results = [
        ...
        { sentiment: Sentiment.NEGATIVE, confidence: 0.75, emotions: [{ name: "anger", score: 0.8 }] },
...
{"commentIndex": 2, "sentiment": "NEGATIVE", "confidence": 0.85, "emotions": [{"name": "anger", "score": 0.9}]}
    ...
    it("should retry low confidence results when enabled", async () => {
      ...
      // The result should have some confidence score (either original or improved)
  ...
  describe("processing configuration", () => {
    ...
    it("should validate configuration bounds", () => {
      analyzer.updateProcessingConfig({
        batchSize: 100, // Too high
        maxRetries: -1, // Invalid
        retryDelay: 50, // Too low
      });

      const stats = analyzer.getProcessingStats();
      // Should keep original values for invalid inputs
      expect(stats.batchSize).toBe(10); // Original value
      expect(stats.maxRetries).toBe(3); // Original value
      expect(stats.retryDelay).toBe(1000); // Original value
    });
  });

  describe("fallback sentiment analysis", () => {
    beforeEach(() => {
      // Mock API to always fail to test fallback
      mockModel.generateContent.mockRejectedValue(new Error("API Error"));
    });
    ...
    it("should normalize and limit emotions correctly", async () => {
      ...
      const mockResponse = JSON.stringify({
        ...
        emotions: [
          ...
          { name: "anticipation", score: 0.5 }, // Should be filtered out (only top 3)
          { name: "invalid_emotion", score: 0.8 }, // Should be filtered out (invalid)
        ],
      });
      ...
      // If emotions are present, check they are valid
      if (result.emotions.length > 0) {
