...
describe("AnalysisOrchestrationService", () => {
  let orchestrationService: AnalysisOrchestrationService;
  let prismaMock: DeepMockProxy<PrismaClient>;
  let jobManagerMock: jest.Mocked<JobManager>;
  ...
  beforeEach(() => {
    ...
    orchestrationService = new AnalysisOrchestrationService(prismaMock as any, jobManagerMock);
  });
  ...
  describe("processAnalysis", () => {
    ...
    beforeEach(() => {
      ...
      (orchestrationService as any).commentPreprocessor.preprocessComments = jest.fn().mockResolvedValue(mockPreprocessResult);
      (orchestrationService as any).sentimentAnalyzer.analyzeBatchSentiment = jest.fn().mockResolvedValue(mockSentimentResult);
      (orchestrationService as any).themeAnalyzer.analyzeThemes = jest.fn().mockResolvedValue(mockThemeResult);
      (orchestrationService as any).summaryGenerator.generateSummary = jest.fn().mockResolvedValue(mockSummaryResult);
    ...
    it("should process analysis successfully", async () => {
      ...
      expect(jobManagerMock.updateJobProgress).toHaveBeenCalledTimes(6); // 5 steps + completion

      // Verify final completion
      expect(jobManagerMock.updateJobProgress).toHaveBeenLastCalledWith("job-123", { progress: 100, currentStep: 5, stepDescription: "Analysis completed" }, JobStatus.COMPLETED);
    });
    ...
    it("should use cached result if available", async () => {
      ...
      prismaMock.analysisResult.update.mockResolvedValue({} as any);

      await orchestrationService.processAnalysis("job-123", "post-123", "user-123", ["comment-1"]);

      // Should update the cached result to link to this job
      expect(prismaMock.analysisResult.update).toHaveBeenCalledWith({
        where: { id: "cached-result-123" },
        data: { jobId: "job-123" },
      });

      // Should mark job as completed immediately
      expect(jobManagerMock.updateJobProgress).toHaveBeenCalledWith(
        "job-123",
        {
          progress: 100,
          currentStep: 5,
  ...
  describe("validateAnalysisPrerequisites", () => {
    it("should return valid for post with sufficient comments", async () => {
      ...
      prismaMock.connectedPlatform.findMany.mockResolvedValue(mockPlatforms as any);
  ...
  describe("estimateAnalysisTime", () => {
    it("should estimate time based on comment count", () => {
      expect(orchestrationService.estimateAnalysisTime(10)).toBe(11); // 10 + (10 * 0.1)
      expect(orchestrationService.estimateAnalysisTime(100)).toBe(20); // 10 + (100 * 0.1)
      expect(orchestrationService.estimateAnalysisTime(5000)).toBe(300); // Capped at 5 minutes
    });
  });
